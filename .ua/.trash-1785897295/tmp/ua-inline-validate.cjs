...
try {
  ...
  graph.layers.forEach(layer => {
    if (!layer.id || !layer.name || !layer.description || !Array.isArray(layer.nodeIds)) issues.push(`Layer '${layer.id || 'unknown'}' has invalid shape`);
  ...
  graph.tour.forEach((step, i) => {
    if (!Number.isInteger(step.order) || !step.title || !step.description || !Array.isArray(step.nodeIds)) issues.push(`Tour step[${i}] has invalid shape`);
